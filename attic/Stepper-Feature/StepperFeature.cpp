/*
  StepperFeature.cpp - Firmata library
  ...
  if (command == STEPPER_DATA) {
    ...
    if (deviceNum < MAX_STEPPERS) {
      if (stepCommand == STEPPER_CONFIG) {
        ...
        if (interface == AsyncStepper::DRIVER || interface == AsyncStepper::TWO_WIRE) {
          stepper[deviceNum] = new AsyncStepper(interface, stepsPerRev, directionPin, stepPin);
        } else if (interface == AsyncStepper::FOUR_WIRE) {
          motorPin3 = argv[7];
          motorPin4 = argv[8];
          ...
          stepper[deviceNum] = new AsyncStepper(interface, stepsPerRev, directionPin, stepPin, motorPin3, motorPin4);
        }
      }
