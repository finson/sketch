/*
  I2CMessages.cpp - Firmata library
...
I2CMessages::I2CMessages() : queryIndex(-1), i2cReadDelayTime(0)
{
  // queryIndex = -1;
  // i2cReadDelayTime = 0;  // default delay time between i2c read request and Wire.requestFrom()
}

boolean I2CMessages::handleFeatureSysex(byte command, byte argc, byte *argv)
{
  switch (command) {
  case I2C_REQUEST:
    if (pmI2C.isEnabled()) {
...
boolean I2CMessages::handleI2CConfig(byte argc, byte *argv)
{
  ...
  pmI2C.enableI2CPins();
  return pmI2C.isEnabled();
}

void I2CMessages::handleI2CRequest(byte argc, byte *argv)
{
  byte mode;
  byte slaveAddress;
  byte data;
...
void I2CMessages::readAndReportData(byte address, int theRegister, byte numBytes) {
  char errorMsg[100] ;
  ...
  if (numBytes == 2) {
    ...
    int value = pmI2C.read16(address, theRegister);
