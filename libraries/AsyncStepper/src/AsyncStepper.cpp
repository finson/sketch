/**
  AsyncStepper is a simple non-blocking stepper motor library
  ...
  AsyncStepper (0.2) Name changed and made into library by Doug Johnson
 ...
 * The interface type: AsyncStepper::DRIVER,
 * AsyncStepper::TWO_WIRE or AsyncStepper::FOUR_WIRE
 ...
 */
AsyncStepper::AsyncStepper(byte interface,
                               int steps_per_rev,
                               byte pin1,
                               byte pin2,
                               byte pin3,
                               byte pin4)
{
  ...
  if (this->interface == AsyncStepper::FOUR_WIRE)
  {
 ...
 */
void AsyncStepper::setStepsToMove(long steps_to_move, int speed, int accel, int decel)
{
  ...
  if (steps_to_move < 0)
  {
    this->direction = AsyncStepper::CCW;
    steps_to_move = -steps_to_move;
  }
  else
  {
    this->direction = AsyncStepper::CW;
  }
  ...
  if (accel == 0 || decel == 0)
  {
    ...
    this->run_state = AsyncStepper::RUN;
    this->accel_count = 0;
    this->running = true;
  ...
  if (steps_to_move == 1)
  {
    ...
    // move one step
    this->accel_count = -1;
    this->run_state = AsyncStepper::DECEL;
  ...
  else if (steps_to_move != 0)
  {
    ...
    {
      this->run_state = AsyncStepper::ACCEL;
    }
  ...
  if (this->running == true && timeDiff >= this->step_delay)
  {
    ...
    switch (this->run_state)
    {
      ...
      case AsyncStepper::RUN:
        ...
        {
          this->run_state = AsyncStepper::STOP;
        }
        // check if we should start deceleration
 ...
 * @private
 */
void AsyncStepper::updateStepPosition()
{
  // increment or decrement the step number,
  // depending on direction:
  if (this->direction == AsyncStepper::CW)
  {
    this->step_number++;
 ...
 */
void AsyncStepper::stepMotor(byte step_num, byte direction)
{
  if (this->interface == AsyncStepper::DRIVER)
  {
  ...
  else if (this->interface == AsyncStepper::TWO_WIRE)
  {
    switch (step_num)
    {
      case 0: /* 01 */
  ...
  else if (this->interface == AsyncStepper::FOUR_WIRE)
  {
    switch (step_num)
    {
      case 0:    // 1010
 ...
 */
byte AsyncStepper::version(void)
{
  return 2;
}
