  ...
  if (command == STEPPER_DATA) {
    ...
    if (deviceNum < MAX_STEPPERS) {
      if (stepCommand == STEPPER_CONFIG) {
        ...
        if (interface == StepperDriver::DRIVER || interface == StepperDriver::TWO_WIRE) {
          stepper[deviceNum] = new StepperDriver(interface, stepsPerRev, directionPin, stepPin);
        } else if (interface == StepperDriver::FOUR_WIRE) {
          motorPin3 = argv[7];
          motorPin4 = argv[8];
          ...
          stepper[deviceNum] = new StepperDriver(interface, stepsPerRev, directionPin, stepPin, motorPin3, motorPin4);
        }
      }
