/**
  StepperDriver is a simple non-blocking stepper motor library
 ...
 * The interface type: StepperDriver::DRIVER,
 * StepperDriver::TWO_WIRE or StepperDriver::FOUR_WIRE
 ...
 */
StepperDriver::StepperDriver(byte interface,
                               int steps_per_rev,
                               byte pin1,
                               byte pin2,
                               byte pin3,
                               byte pin4)
{
  ...
  if (this->interface == StepperDriver::FOUR_WIRE)
  {
 ...
 */
void StepperDriver::setStepsToMove(long steps_to_move, int speed, int accel, int decel)
{
  ...
  if (steps_to_move < 0)
  {
    this->direction = StepperDriver::CCW;
    steps_to_move = -steps_to_move;
  }
  else
  {
    this->direction = StepperDriver::CW;
  }
  ...
  if (accel == 0 || decel == 0)
  {
    ...
    this->run_state = StepperDriver::RUN;
    this->accel_count = 0;
    this->running = true;
  ...
  if (steps_to_move == 1)
  {
    ...
    // move one step
    this->accel_count = -1;
    this->run_state = StepperDriver::DECEL;
  ...
  else if (steps_to_move != 0)
  {
    ...
    {
      this->run_state = StepperDriver::ACCEL;
    }
  ...
  if (this->running == true && timeDiff >= this->step_delay)
  {
    ...
    switch (this->run_state)
    {
      ...
      case StepperDriver::RUN:
        ...
        {
          this->run_state = StepperDriver::STOP;
        }
        // check if we should start deceleration
 ...
 * @private
 */
void StepperDriver::updateStepPosition()
{
  // increment or decrement the step number,
  // depending on direction:
  if (this->direction == StepperDriver::CW)
  {
    this->step_number++;
 ...
 */
void StepperDriver::stepMotor(byte step_num, byte direction)
{
  if (this->interface == StepperDriver::DRIVER)
  {
  ...
  else if (this->interface == StepperDriver::TWO_WIRE)
  {
    switch (step_num)
    {
      case 0: /* 01 */
  ...
  else if (this->interface == StepperDriver::FOUR_WIRE)
  {
    switch (step_num)
    {
      case 0:    // 1010
 ...
 */
byte StepperDriver::version(void)
{
  return 2;
}
