 ...
 * It's very possible to have devices that expect BE byte order attached to
 * LE hosts, and so these methods allow the user to easily match the device's
 * needs.  The byte order of the host is determined at compile time and the
 * code is adjusted to select the order in which the bytes are addressed
 * locally.
 */
class Port {
...
public:
  ...
  virtual void writeUInt8(int addr, uint8_t reg, uint8_t *valueAddress) = 0;
  virtual void writeUInt16LE(int addr, uint8_t reg, uint8_t *valueAddress) = 0;
  virtual void writeUInt16BE(int addr, uint8_t reg, uint8_t *valueAddress) = 0;
  virtual void writeUInt32LE(int addr, uint8_t reg, uint8_t *valueAddress) = 0;
  virtual void writeUInt32BE(int addr, uint8_t reg, uint8_t *valueAddress) = 0;
